...
int main(int argc, char** argv)
{
	...
	IplImage *diff = cvCreateImage(cvGetSize(img2_temp),img2_temp->depth,img2_temp->nChannels);
	cvCopy(img2_temp, diff);
	...
	IplImage *gray32 = cvCreateImage(size,d,1);//»Ò¶ÈÍ¼
	cvCvtColor(ssim_map, gray32, CV_BGR2GRAY);//cvCvtColor(src,des,CV_BGR2GRAY)  

	IplImage *gray8=cvCreateImage(size, IPL_DEPTH_8U, 1);
	cvConvertScale(gray32, gray8, 255);

	IplImage *gray1=cvCreateImage(size, IPL_DEPTH_8U, 1);
	cvThreshold(gray8, gray1, 254, 255, CV_THRESH_BINARY_INV);
	cvNamedWindow("thresh");
	cvShowImage("thresh", gray1);

	CvMemStorage * storage = cvCreateMemStorage(0);  
	CvSeq * contour = 0;
	int Num = cvFindContours(gray1, storage, &contour, sizeof(CvContour), 
	...
	for(;contour!=0;contour=contour->h_next)  
	{
		...
		cvRectangle( diff, cvPoint( rect.x, rect.y ),cvPoint( rect.x + rect.width, rect.y + rect.height ), cvScalar(0,0,255), 2 );
	}  
	cvNamedWindow("diff");
